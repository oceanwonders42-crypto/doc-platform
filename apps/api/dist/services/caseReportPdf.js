...
exports.getCaseReportData = getCaseReportData;
exports.buildCaseReportPdf = buildCaseReportPdf;
/**
...
const caseInsights_1 = require("./caseInsights");
const MARGIN = 50;
...
async function getCaseReportData(caseId, firmId) {
    ...
    let recRows = [];
    if (docIds.length > 0) {
        const { rows } = await pg_1.pgPool.query(`select document_id, doc_type, summary, insurance_fields from document_recognition where document_id = any($1)`, [docIds]);
        recRows = rows;
    }
    ...
    const { insights } = await (0, caseInsights_1.getCaseInsights)(caseId, firmId);
    return {
...
function stringifyInsuranceFields(ins) {
    if (ins == null)
        return "";
    ...
    const o = ins;
    const parts = [];
    if (o.insuranceCompany != null)
        parts.push(`Carrier: ${String(o.insuranceCompany)}`);
    if (o.settlementOffer != null)
        parts.push(`Settlement offer: $${Number(o.settlementOffer).toLocaleString()}`);
    if (o.policyLimits != null)
        parts.push(`Policy limits: ${JSON.stringify(o.policyLimits)}`);
    if (o.letterDate != null)
        parts.push(`Letter date: ${String(o.letterDate)}`);
    const rest = Object.entries(o).filter(([k]) => !["insuranceCompany", "settlementOffer", "policyLimits", "letterDate"].includes(k));
...
async function buildCaseReportPdf(caseId, firmId) {
    ...
    if (data.timelineEvents.length === 0) {
        ...
        for (const e of data.timelineEvents) {
            ...
            if (e.diagnosis || e.procedure)
                drawLine(`   ${[e.diagnosis, e.procedure].filter(Boolean).join(" · ")}`, { fontSize: FONT_SIZE - 1 });
            if (e.amount)
    ...
    if (insuranceDocs.length === 0) {
        ...
        for (const d of insuranceDocs) {
            ...
            if (d.insurance_fields != null) {
                ...
                if (str)
                    drawBlock(str, { fontSize: FONT_SIZE - 1 });
            }
    ...
    if (data.insights.length === 0) {
        ...
        for (const i of data.insights) {
            ...
            if (i.detail)
                drawBlock(i.detail, { fontSize: FONT_SIZE - 1 });
        }
    }
