...
exports.createProviderSuggestion = createProviderSuggestion;
/**
...
function extractFax(text) {
    const m = text.match(FAX_LABEL);
    if (m)
        return extractFirstPhone(m[0]);
    return null;
}
...
function extractAddress(text) {
    const addrMatch = text.match(/\b(\d{1,6}\s+[\w\s]+(?:street|st|avenue|ave|road|rd|blvd|drive|dr|lane|ln|way|place|pl)\b[^.]{0,80})/i);
    if (addrMatch)
        return normalizeWhitespace(addrMatch[1]).slice(0, 200);
    const cityState = text.match(/\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*),\s*([A-Z]{2})\s+\d{5}(-\d{4})?\b/);
    if (cityState)
        return normalizeWhitespace(cityState[0]).slice(0, 200);
    return null;
}
...
async function createProviderSuggestion(firmId, documentId, extracted) {
    const name = extracted.providerName || extracted.facilityName;
    if (!name)
        return;
    await pg_1.pgPool.query(`INSERT INTO document_provider_suggestion (id, firm_id, document_id, extracted_name, facility_name, specialty, phone, fax, address)
