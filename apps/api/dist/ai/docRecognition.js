...
function getPdfjs() {
    ...
    throw new Error("PDF text extraction unavailable: pdfjs-dist not installed or wrong build. Run: pnpm -C apps/api add pdfjs-dist@^3");
}
...
function classifyAndExtract(textRaw) {
    ...
    let docType = "unknown";
    if (lower.includes("medical record") || lower.includes("patient") || lower.includes("diagnosis"))
        docType = "medical_record";
    else if (lower.includes("police") || lower.includes("incident report") || lower.includes("offense"))
        docType = "police_report";
    else if (lower.includes("invoice") || lower.includes("amount due") || lower.includes("balance"))
        docType = "invoice";
    else if (lower.includes("demand") && lower.includes("settlement"))
        docType = "demand_letter";
    else if (lower.includes("intake") || lower.includes("new client") || lower.includes("questionnaire"))
        docType = "client_intake";
