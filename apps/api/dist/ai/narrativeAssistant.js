...
function formatQuestionnaireBlock(q) {
    if (!q)
        return "";
    const lines = [];
    if (q.mainInjuries?.trim())
        lines.push("Main injuries: " + q.mainInjuries.trim());
    if (q.treatmentHighlights?.trim())
        lines.push("Treatment highlights: " + q.treatmentHighlights.trim());
    ...
    if (q.currentDemandAmount?.trim())
        lines.push("Current demand amount: " + q.currentDemandAmount.trim());
    if (q.keyLiabilityFacts?.trim())
        lines.push("Key liability facts: " + q.keyLiabilityFacts.trim());
    if (lines.length === 0)
        return "";
...
async function generateNarrative(input) {
    ...
    const docs = docIds.length > 0
        ...
        : [];
    const extractedByDoc = new Map(docs.map((d) => [d.id, d.extractedFields || {}]));
    const injuries = [];
    const diagnoses = [];
    const procedures = [];
    events.forEach((e) => {
        if (e.diagnosis && !diagnoses.includes(e.diagnosis))
            diagnoses.push(e.diagnosis);
        if (e.procedure && !procedures.includes(e.procedure))
            procedures.push(e.procedure);
    });
    docs.forEach((d) => {
        const ef = extractedByDoc.get(d.id) || {};
        const med = ef.medicalRecord;
        if (med && typeof med === "object") {
            ...
            if (proc) {
                ...
                arr.forEach((x) => {
                    const s = String(x).trim();
                    if (s && !procedures.includes(s))
                        procedures.push(s);
                });
            }
        }
        if (ef.injuries && Array.isArray(ef.injuries)) {
            ef.injuries.forEach((s) => {
    ...
    const timelineLines = events.map((e) => {
        ...
        if (e.provider)
            parts.push("Provider: " + e.provider);
        if (e.diagnosis)
            parts.push("Dx: " + e.diagnosis);
        if (e.procedure)
            parts.push("Proc: " + e.procedure);
        if (e.amount)
            parts.push("Amount: " + e.amount);
    ...
    if (events.length === 0)
        warnings.push("No timeline events found for this case; draft may be generic.");
    if (diagnoses.length === 0 && procedures.length === 0)
        warnings.push("No diagnoses or procedures in timeline; consider adding medical records.");
    ...
    const prompt = "You are helping draft narrative sections for a demand package. Output draft language only-no legal advice, no recommendations. Do not invent facts. Use only the case information provided below. If critical information is missing, use [BRACKETED PLACEHOLDERS] (e.g. [CLIENT NAME], [DATE OF INCIDENT], [INSURER NAME]).\n\n" +
