 ...
 */
function classifyOnyx(extractedText, filename = "") {
    ...
    if (erHits >= 1)
        candidates.push({ docType: "er_record", hits: erHits, signals: ER_KEYWORDS.filter((k) => combined.includes(k)) });
    const imgHits = countHits(combined, IMAGING_KEYWORDS);
    if (imgHits >= 1)
        candidates.push({ docType: "imaging_report", hits: imgHits, signals: IMAGING_KEYWORDS.filter((k) => combined.includes(k)) });
    const opHits = countHits(combined, OPERATIVE_KEYWORDS);
    if (opHits >= 2)
        candidates.push({ docType: "operative_report", hits: opHits, signals: OPERATIVE_KEYWORDS.filter((k) => combined.includes(k)) });
    const physHits = countHits(combined, PHYSICIAN_NOTES_KEYWORDS);
    if (physHits >= 1)
        candidates.push({ docType: "physician_notes", hits: physHits, signals: PHYSICIAN_NOTES_KEYWORDS.filter((k) => combined.includes(k)) });
    const pcpHits = countHits(combined, PCP_KEYWORDS);
    if (pcpHits >= 1)
        candidates.push({ docType: "pcp_notes", hits: pcpHits, signals: PCP_KEYWORDS.filter((k) => combined.includes(k)) });
    const ptHits = countHits(combined, THERAPY_PT_KEYWORDS);
    if (ptHits >= 1)
        candidates.push({ docType: "therapy_pt_notes", hits: ptHits, signals: THERAPY_PT_KEYWORDS.filter((k) => combined.includes(k)) });
    const billHits = countHits(combined, MEDICAL_BILL_KEYWORDS);
    if (billHits >= 2)
        candidates.push({ docType: "medical_bill", hits: billHits, signals: MEDICAL_BILL_KEYWORDS.filter((k) => combined.includes(k)) });
    const ledgerHits = countHits(combined, LEDGER_STATEMENT_KEYWORDS);
    if (ledgerHits >= 2)
        candidates.push({ docType: "ledger_statement", hits: ledgerHits, signals: LEDGER_STATEMENT_KEYWORDS.filter((k) => combined.includes(k)) });
    const insHits = countHits(combined, INSURANCE_CORRESPONDENCE_KEYWORDS);
    if (insHits >= 1)
        candidates.push({ docType: "insurance_correspondence", hits: insHits, signals: INSURANCE_CORRESPONDENCE_KEYWORDS.filter((k) => combined.includes(k)) });
...
function onyxToLegacyDocType(onyx) {
    const map = {
