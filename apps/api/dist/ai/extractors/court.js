...
function extractCourt(text) {
    ...
    if (caseMatch)
        out.caseNumber = caseMatch[1].trim();
    const courtMatch = t.match(COURT_NAME);
    if (courtMatch)
        out.courtName = courtMatch[1].trim();
    const countyMatch = t.match(COUNTY);
    if (countyMatch)
        out.county = countyMatch[1].trim();
    const plaintiffMatch = t.match(PLAINTIFF);
    const defendantMatch = t.match(DEFENDANT);
    if (plaintiffMatch || defendantMatch) {
        out.parties = {};
        if (plaintiffMatch)
            out.parties.plaintiff = plaintiffMatch[1].trim().slice(0, 200);
        if (defendantMatch)
            out.parties.defendant = defendantMatch[1].trim().slice(0, 200);
    }
    const judgeMatch = t.match(JUDGE);
    if (judgeMatch)
        out.judge = judgeMatch[1].trim();
    const filingMatch = t.match(FILING_DATE);
    if (filingMatch)
        out.filingDate = filingMatch[1].trim();
    const hearingMatch = t.match(HEARING_DATE);
    if (hearingMatch)
        out.hearingDate = hearingMatch[1].trim();
    const deadlines = [];
    let dm;
    while ((dm = DEADLINE.exec(t)) !== null) {
        const d = dm[1].trim();
        if (d && !deadlines.includes(d))
            deadlines.push(d);
    }
    if (deadlines.length)
