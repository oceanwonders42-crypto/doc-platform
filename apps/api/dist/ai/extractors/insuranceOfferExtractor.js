...
exports.extractInsuranceOfferFields = extractInsuranceOfferFields;
/**
...
async function extractInsuranceOfferFields(args) {
    ...
    try {
        ...
        if (typeof parsed.settlementOffer === "number" && Number.isFinite(parsed.settlementOffer)) {
            ...
            if (Number.isFinite(num))
                result.settlementOffer = Math.round(num);
        }
        ...
        if (Array.isArray(parsed.warnings)) {
            for (const w of parsed.warnings) {
                if (typeof w === "string" && w.trim())
                    warnings.push(w.trim().slice(0, 300));
            }
        }
    }
    catch (err) {
