...
exports.extractInsurance = extractInsurance;
...
function extractInsurance(text) {
    ...
    if (policyMatch)
        out.policyNumber = policyMatch[1].trim();
    const insurerMatch = t.match(INSURER);
    if (insurerMatch)
        out.insurerName = insurerMatch[1].trim().slice(0, 120);
    const adjusterMatch = t.match(ADJUSTER_NAME);
    if (adjusterMatch)
        out.adjusterName = adjusterMatch[1].trim();
    const emailMatch = t.match(ADJUSTER_EMAIL);
    if (emailMatch)
        out.adjusterEmail = emailMatch[0];
    const phoneMatch = t.match(ADJUSTER_PHONE);
    if (phoneMatch)
        out.adjusterPhone = `${phoneMatch[1]}-${phoneMatch[2]}-${phoneMatch[3]}`;
    const lossMatch = t.match(LOSS_DATE);
    if (lossMatch)
        out.lossDate = lossMatch[1].trim();
    const letterMatch = t.match(LETTER_DATE);
    if (letterMatch)
        out.letterDate = letterMatch[1].trim();
    if (DENIAL.test(t))
        out.coverageDecision = "denied";
    else if (ACCEPTED.test(t))
        out.coverageDecision = "accepted";
    const offerMatch = t.match(OFFER_AMT);
    if (offerMatch)
        out.offerAmount = offerMatch[1].trim();
    const limits = {};
