...
exports.extractInsuranceLetter = extractInsuranceLetter;
...
const SETTLEMENT_OFFER_EXPLICIT = /\b(?:settlement\s+offer|offer\s+amount|we\s+offer|our\s+offer|offer\s*[:\-])\s*\$?\s*([0-9,]+(?:\.\d{2})?)/i;
const SETTLEMENT_IN_AMOUNT = /\b(?:settlement|offer|payment)\s+(?:in\s+the\s+)?(?:amount|sum)\s+of\s*\$?\s*([0-9,]+(?:\.\d{2})?)/i;
...
function extractInsuranceLetter(text) {
    const out = {};
    const t = text.replace(/\s+/g, " ").trim();
    const tFull = text;
    const adjusterMatch = t.match(ADJUSTER_NAME);
    if (adjusterMatch)
        out.adjusterName = adjusterMatch[1].trim();
    const companyMatch = t.match(INSURANCE_COMPANY);
    if (companyMatch)
        out.insuranceCompany = companyMatch[1].trim().slice(0, 120);
    const claimMatch = t.match(CLAIM_NUMBER);
    if (claimMatch)
        out.claimNumber = claimMatch[1].trim();
    const settlementMatch = t.match(SETTLEMENT_OFFER_EXPLICIT) ||
        ...
        t.match(OFFER_AMT);
    if (settlementMatch)
        out.settlementOffer = settlementMatch[1].trim();
    const limits = {};
