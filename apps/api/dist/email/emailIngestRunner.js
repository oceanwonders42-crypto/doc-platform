...
function shouldIngestAttachment(filename, mimeType) {
    const lower = (filename || "").toLowerCase();
    if (lower.endsWith(".pdf"))
        return true;
    const mt = (mimeType || "").toLowerCase();
    if (mt === "application/pdf" || mt.startsWith("application/pdf;"))
        return true;
    return false;
}
...
async function runEmailPollOnce() {
    ...
    for (const mb of mailboxes) {
        console.log(`[email] polling mailbox id=${mb.id} provider=${mb.provider} user=${mb.imap_username} host=${mb.imap_host}`);
        try {
            if (mb.provider === "imap") {
...
async function handleImapMailbox(mb) {
    ...
    console.log(`[email] imap returned messages=${messages.length} highestUid=${highestUid ?? "null"} lastUidWas=${lastUid ?? "null"}`);
  ...
  `, [
        ...
        for (const a of attachments) {
            ...
            if (isPdf) {
                console.log(`[email] ingesting PDF ${a.filename} subject=${JSON.stringify(m.subject || "")} from=${m.fromEmail || ""}`);
            }
