...
async function main() {
  ...
  try {
    ...
    assert.equal(marchUsage.demandCount, 1, "Previous-month saved output should remain scoped to its own month.");
    ...
    assert.equal(recorded.demandPackage.status, "pending_dev_review");
    assert.equal(recorded.demandPackage.generatedDocId, `doc-new-${suffix}`);
    assert.equal(recorded.demandPackage.generatedAt?.toISOString(), aprilGeneratedAt.toISOString());
    assert.equal(recorded.usage.yearMonth, "2026-04");
    assert.equal(recorded.usage.demandCount, 2, "Recording one saved demand output should increase the clean monthly count by one.");
    ...
    assert.equal(aprilAfter.demandCount, 2, "The original month should retain the same clean demand count after regeneration.");
    ...
    await prisma.demandPackage.deleteMany({
      where: {
        id: {
          in: [existingAprilPackageId, draftPackageId, previousMonthPackageId, docOnlyPackageId, dateOnlyPackageId],
        },
      },
    });
