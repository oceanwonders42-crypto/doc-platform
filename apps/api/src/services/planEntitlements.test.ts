...
async function main() {
  ...
  assert.equal(getCanonicalEntitlementPlanSlug("essential"), "essential");
  assert.equal(getCanonicalEntitlementPlanSlug("growth"), "growth");
  assert.equal(getCanonicalEntitlementPlanSlug("premium"), "premium");
  assert.equal(
    getCanonicalEntitlementPlanSlug("paperless_transition"),
    "paperless_transition"
  );
  assert.equal(getCanonicalEntitlementPlanSlug("starter"), "essential");
  assert.equal(getCanonicalEntitlementPlanSlug("professional"), "growth");
  assert.equal(getCanonicalEntitlementPlanSlug("enterprise"), "premium");
  ...
  assert.equal(getRecurringEntitlementPlanSlug("enterprise"), "premium");
  assert.equal(getRecurringEntitlementPlanSlug("paperless_transition"), null);
  ...
  assert.throws(
    () => getRecurringEntitlementPlanSlug("scale"),
    /Invalid plan label/
  );
  assert.throws(
    () => getRecurringEntitlementPlanSlug("custom"),
    /Invalid plan label/
  );
