...
async function main() {
  ...
  assert.equal(
    getCanonicalPolicyPlanSlug("paperless_transition"),
    "paperless_transition"
  );

  assert.equal(getCanonicalPolicyPlanSlug("starter"), "essential");
  assert.equal(getCanonicalPolicyPlanSlug("professional"), "growth");
  assert.equal(getCanonicalPolicyPlanSlug("enterprise"), "premium");

  assert.equal(getCanonicalPolicyPlanSlug("core"), null);
