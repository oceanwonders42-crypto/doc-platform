...
export async function getMonthlyDemandUsage(firmId: string, at: Date = new Date()): Promise<MonthlyDemandUsage> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const demandPackage = await tx.demandPackage.update({
      ...
      data: {
        ...
        status: params.status ?? "pending_dev_review",
      },
      select: {
        id: true,
        firmId: true,
    ...
    const usage = await countTrueDemandsForMonth(tx as DemandUsageReader, params.firmId, firstGeneratedAt);
