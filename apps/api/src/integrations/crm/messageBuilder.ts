...
export type CaseIntelligenceActionType =
  | "document_routed"
  | "document_approved"
  | "narrative_generated"
  | "timeline_rebuilt";
...
export type BuildMessageParams = {
  ...
  sourceDocuments?: Array<{ id: string; fileName?: string | null }>;
};

const ACTION_LABELS: Record<CaseIntelligenceActionType, string> = {
...
export function buildCaseIntelligenceMessage(params: BuildMessageParams): { title: string; bodyMarkdown: string } {
  const {
    caseId,
    actionType,
    documentId,
  ...
  if (actionType === "narrative_generated" && narrativeExcerpt && narrativeExcerpt.trim()) {
    ...
    if (narrativeExcerpt.length > 2000) lines.push("…");
    lines.push("");
  }
