...
function LoginContent() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    if (!base) {
      setError("API URL not configured. Set NEXT_PUBLIC_API_URL (e.g. http://localhost:4000).");
      setLoading(false);
      return;
    }
  ...
  const redirectToOAuth = useCallback(
    (provider: "google" | "microsoft") => {
      if (!base) {
        setError("API URL not configured. Set NEXT_PUBLIC_API_URL.");
        return;
      }
  ...
  return (
    <div className="login-page dashboard-theme">
      <div className="login-card onyx-card">
        ...
        <p className="login-demo-hint" style={{ marginTop: "0.75rem", fontSize: "0.8125rem", color: "var(--onyx-text-muted)" }}>
