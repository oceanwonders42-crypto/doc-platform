...
import AIExtractionPanel from "../../dashboard/review/AIExtractionPanel";

async function fetchDocumentName(documentId: string): Promise<string> {
  const res = await fetch(
    `${process.env.DOC_WEB_BASE_URL ?? ""}/api/documents/${documentId}/recognition`,
    { cache: "no-store" }
  ).catch(() => null);
  if (!res?.ok) return "";
  const data = (await res.json().catch(() => ({}))) as { document?: { originalName?: string } };
  return (data.document?.originalName ?? "").trim() || "Document";
}
...
export async function generateMetadata({
  ...
  const name = await fetchDocumentName(id);
  return { title: name === "Document" ? "Document" : `Document - ${name}` };
}
...
export default async function DocumentPage({
  ...
  const docName = recognition.originalName || `Document ${id}`;
    ...
    <main style={{ padding: 24, margin: "0 auto", fontFamily: "system-ui, -apple-system" }}>
      ...
      >
        <div>
          ...
          <AIExtractionPanel
            doc={{
              ...(recognition.extractedFields || {}),
              extractedFields: recognition.extractedFields,
              insuranceFields: recognition.insuranceFields,
