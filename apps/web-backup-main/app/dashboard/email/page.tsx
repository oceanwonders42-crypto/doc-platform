...
export default function EmailDashboardPage() {
  ...
  return (
    <main style={{ padding: 24, maxWidth: 1000, margin: "0 auto", fontFamily: "system-ui, -apple-system" }}>
      <Breadcrumbs items={[{ label: "Dashboard", href: "/dashboard" }, { label: "Email intake" }]} />
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 20, marginTop: 0 }}>Email intake</h1>
                ...
                {mailboxes.map((mb) => (
                  <tr key={mb.id} style={{ borderTop: "1px solid #eee" }}>
                    <td style={{ padding: "10px 12px", fontSize: 14 }}>
                      <span
                        style={{
                          ...
                          fontSize: 12,
                          fontWeight: 500,
                          background: mb.status === "active" ? "var(--status-success-bg)" : "#f5f5f5",
                          color: mb.status === "active" ? "var(--status-success-text)" : "#666",
                        }}
                      >
                    ...
                    <td style={{ padding: "10px 12px", fontSize: 13, color: "#666" }}>
                      {mb.last_sync_at ? formatTimestamp(mb.last_sync_at) : "—"}
                      {mb.last_error && (
                        <div style={{ marginTop: 4, color: "var(--status-error-text)", fontSize: 12 }} title={mb.last_error}>
              ...
              <tbody>
                {recentIngests.map((ing) => (
                  <tr key={ing.id} style={{ borderTop: "1px solid #eee" }}>
                    ...
                    <td style={{ padding: "10px 12px", fontSize: 13, color: "#666" }}>
                      {formatTimestamp(ing.receivedAt)}
                    </td>
