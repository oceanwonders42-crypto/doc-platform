...
function formatKeyFields(fields: Record<string, unknown> | null | undefined): string {
  if (!fields || typeof fields !== "object") return "";
  const parts: string[] = [];
  const court = fields.court as Record<string, unknown> | undefined;
  const insurance = fields.insurance as Record<string, unknown> | undefined;
  if (court && typeof court === "object") {
    if (court.caseNumber) parts.push(`Case #${court.caseNumber}`);
    if (court.courtName) parts.push(String(court.courtName));
    if (court.filingDate) parts.push(String(court.filingDate));
  }
  if (insurance && typeof insurance === "object") {
    if (insurance.claimNumber) parts.push(`Claim #${insurance.claimNumber}`);
    if (insurance.offerAmount != null) parts.push(`$${insurance.offerAmount}`);
    if (insurance.insurerName) parts.push(String(insurance.insurerName));
  }
  if (fields.caseNumber && !parts.length) parts.push(String(fields.caseNumber));
  if (fields.clientName && !parts.some((p) => p.includes(String(fields.clientName)))) parts.push(String(fields.clientName));
  ...
  return (
    <section>
      ...
      <div style={{ marginBottom: 24, padding: 16, border: "1px solid #e5e5e5", borderRadius: 12 }}>
        ...
        {uploadMsg && <div style={{ marginTop: 8, fontSize: 14, color: "#666" }}>{uploadMsg}</div>}
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr style={{ textAlign: "left", borderBottom: "1px solid #eee", background: "#fafafa" }}>
                ...
                <th style={{ padding: "12px 16px", fontSize: 14, fontWeight: 600 }}>Type</th>
                <th style={{ padding: "12px 16px", fontSize: 14, fontWeight: 600 }}>Uploaded</th>
                <th style={{ padding: "12px 16px", fontSize: 14, fontWeight: 600 }}>Key fields</th>
            ...
            <tbody>
              {items.map((d) => {
                const uploadedDate = d.ingestedAt ?? d.createdAt;
                const keyFields = formatKeyFields(d.extractedFields);
                return (
                  <tr key={d.id} style={{ borderBottom: "1px solid #f3f3f3" }}>
                    <td style={{ padding: "12px 16px" }}>
                      ...
                      <span style={{ color: "#888", marginLeft: 8 }}>{formatFileSize(d.fileSizeBytes)}</span>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      {d.docType ? (
                        <span style={{ textTransform: "capitalize" }}>{String(d.docType).replace(/_/g, " ")}</span>
                      ) : (
