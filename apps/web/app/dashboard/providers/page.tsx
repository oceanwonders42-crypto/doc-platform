...
type ProvidersListResponse = { items?: Provider[] };
...
export default function ProvidersPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${base}/providers?${params.toString()}`, { headers: getAuthHeader(), ...getFetchOptions() })
      ...
      .then((res: unknown) => {
        if (isProvidersListResponse(res) && res.items) setItems(res.items);
  ...
  return (
    <div style={{ padding: "0 1.5rem 1.5rem" }}>
      <PageHeader
        ...
        description="Healthcare providers linked to your cases"
        action={
          <input
            type="search"
      ...
      {loading ? (
        ...
        <div className="onyx-card" style={{ padding: "2.5rem", textAlign: "center", maxWidth: "28rem" }}>
          ...
          <p style={{ margin: "0.5rem 0 0", fontSize: "0.875rem", color: "var(--onyx-text-muted)", lineHeight: 1.5 }}>
            Providers appear here when linked to cases or when you create records requests.
          </p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", justifyContent: "center", marginTop: "1.25rem" }}>
            <Link href="/dashboard/cases" className="onyx-btn-primary" style={{ textDecoration: "none" }}>View cases</Link>
            <Link href="/dashboard/records-requests/new" className="onyx-btn-secondary" style={{ textDecoration: "none" }}>New records request</Link>
          </div>
        </div>
      ) : (
