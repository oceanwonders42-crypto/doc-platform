...
export default function DashboardHomePage() {
  ...
  const anyLoading = loadingSummary || loadingCases || loadingActivity || loadingQueue;
    ...
    <div className="dashboard-page" style={{ padding: "0 var(--onyx-content-padding) var(--onyx-content-padding)" }}>
      ...
      >
        ...
        <Link href="/dashboard/documents" className="onyx-link" style={{ textDecoration: "none" }}>
          <StatsWidget
            label={t("dashboard.documentsProcessed")}
            value={loadingSummary ? "" : (summary?.docsProcessedThisMonth ?? 0)}
            subtext={loadingSummary ? undefined : t("dashboard.thisMonth")}
        ...
        <Link href="/dashboard/documents" className="onyx-link" style={{ textDecoration: "none" }}>
          <StatsWidget
            ...
            subtext={loadingSummary ? undefined : t("dashboard.notYetRouted")}
            skeleton={loadingSummary}
          />
        </Link>
        <Link href="/dashboard/records-requests" className="onyx-link" style={{ textDecoration: "none" }}>
          <StatsWidget
            label={t("dashboard.recordsRequests")}
            value={loadingSummary ? "" : (summary?.recordsRequestsCreatedThisMonth ?? 0)}
            subtext={loadingSummary ? undefined : t("dashboard.thisMonth")}
            skeleton={loadingSummary}
          />
        </Link>
        <div className="onyx-card" style={{ padding: "1.375rem 1.25rem", borderRadius: "var(--onyx-radius-lg)" }}>
          ...
          {!loadingQueue && (
            <p style={{ margin: "0.25rem 0 0", fontSize: "0.75rem", color: "var(--onyx-text-muted-soft)" }}>
              {pendingCount > 0 ? t("dashboard.inQueueSubtext") : t("dashboard.allClear")}
            </p>
          )}
        </div>
      </div>
      ...
      {/* Main content grid: Recent activity + Trends */}
      <div
        style={{
          display: "grid",
