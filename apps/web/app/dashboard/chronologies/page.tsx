...
export default function ChronologiesPage() {
  ...
  const columns: Column<CaseItem>[] = [
    {
      ...
      header: "Client",
      render: (row) => (
        <Link href={`/dashboard/cases/${row.id}?tab=chronology`} className="onyx-link" style={{ fontWeight: 500 }}>
    ...
    },
    {
      ...
      header: "",
      render: (row) => (
        <Link href={`/dashboard/cases/${row.id}?tab=chronology`} className="onyx-link" style={{ fontSize: "0.875rem" }}>
          View chronology
        </Link>
      ),
    },
  ...
  return (
    <div style={{ padding: "0 1.5rem 1.5rem" }}>
      <PageHeader
        breadcrumbs={[{ label: "Chronologies" }]}
        title="Chronologies"
        description="Case timelines and treatment chronology. Open a case to view or rebuild its chronology."
        action={
      ...
      {loading ? (
        ...
        <div className="onyx-card" style={{ overflow: "hidden" }}>
          <DataTable
            ...
            emptyMessage={
              items.length === 0
                ? "No cases yet. Create cases and add documents to build chronologies."
