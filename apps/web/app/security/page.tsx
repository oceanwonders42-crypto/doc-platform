    ...
    <main className="min-h-screen bg-[#f3f4f6] text-[#0a0a0a]">
      ...
      <section className="relative overflow-hidden border-b border-[#e5e7eb]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-24">
          ...
          <Link href="/demo" className="mt-8 inline-flex rounded-full bg-[#0a0a0a] px-6 py-3 text-sm font-black text-white transition hover:bg-[#2563eb]">
            Book a Demo
          </Link>
        </div>
      </section>

      <section className="mx-auto grid max-w-7xl gap-6 px-5 py-16 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {controls.map((control) => (
            <div key={control} className="rounded-2xl border border-[#e5e7eb] bg-white p-5 shadow-sm">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-16 lg:px-8">
        <div className="grid gap-4 lg:grid-cols-3">
          {cards.map(([title, body]) => (
            <article key={title} className="rounded-3xl border border-[#e5e7eb] bg-white p-6 shadow-[0_14px_35px_rgba(15,23,42,0.07)]">
