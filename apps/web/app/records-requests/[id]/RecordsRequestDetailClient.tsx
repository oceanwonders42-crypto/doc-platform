...
export function RecordsRequestDetailClient({ id }: { id: string }) {
  ...
  const fmt = (s: string | null | undefined) => (s ? new Date(s).toLocaleDateString("en-US") : "—");
  const dateRange =
    item.dateFrom || item.dateTo
      ? [item.dateFrom ? fmt(item.dateFrom) : null, item.dateTo ? fmt(item.dateTo) : null]
          .filter(Boolean)
          .join(" – ")
      : "All dates";
  ...
  return (
    <section style={{ maxWidth: 720, margin: "0 auto" }}>
      <div style={{ marginBottom: 24 }}>
        <Link
          href={item.caseId ? `/cases/${item.caseId}/records-requests` : "/dashboard"}
      ...
      <h1 style={{ fontSize: 22, fontWeight: 600, marginBottom: 24 }}>Records Request</h1>
